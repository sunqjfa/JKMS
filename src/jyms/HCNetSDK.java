...
//    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("\\lib\\HCNetSDK", HCNetSDK.class);
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(System.getProperty("user.dir")+File.separator+"lib"+File.separator+"HCNetSDK", HCNetSDK.class);
    //将DLL文件放置在和JNA.jar同一目录下，只写DLL文件的名字即可。可以运行程序，但是在设备管理注册添加设备时出错。还是找不到NET_DVR_Login_V40
    //HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("HCNetSDK", HCNetSDK.class);
//    public  String HCNetSDKDllPath = getClass().getResource("/").getPath()+"HCNetSDK.dll";
    
    
    /***宏定义***/
    //常量
    //鱼球联动时鱼眼中的参数
    public static final int MAX_CALIB_PT = 6;	//最大标定点个数
    public static final int NET_DVR_DEV_ADDRESS_MAX_LEN = 129;  //设备地址最大长度
    public static final int NET_DVR_LOGIN_USERNAME_MAX_LEN = 64;//设备登录用户名最大长度
    public static final int NET_DVR_LOGIN_PASSWD_MAX_LEN = 64;  //设备登录密码最大长度
...
//9000 IPC接入
    ...
    public static final int STREAM_ID_LEN = 32;// 流ID标识长度 
    ...
    public static final int NET_DVR_PLAYSETTIME  = 26;// 按绝对时间定位 
    public static final int NET_DVR_PLAYGETTOTALLEN =  27;// 获取按时间回放对应时间段内的所有文件的总长度 
    public static final int NET_DVR_PLAY_FORWARD  = 29;// 倒放切换为正放 
    public static final int NET_DVR_PLAY_REVERSE  = 30;// 正放切换为倒放 
    public static final int NET_DVR_SET_TRANS_TYPE  = 32;// 设置转封装类型 
    public static final int NET_DVR_PLAY_CONVERT =  33;// 回放转码 
    public static final int NET_DVR_START_DRAWFRAME  = 34;// 开始抽帧回放 
    public static final int NET_DVR_STOP_DRAWFRAME  = 35 ;//停止抽帧回放 
...
//用于NET_DVR_SetDVRConfig和NET_DVR_GetDVRConfig,注意其对应的配置结构
    ...
    public static final int NET_DVR_SET_TIMECORRECT = 432;//校时配置(针对网络摄像机)
...
//设备编码类型配置(NET_DVR_COMPRESSION_AUDIO结构)
    ...
    //设置手动跟踪定位
    public static final int NET_DVR_CONTROL_PTZ_MANUALTRACE = 3316;//设置手动跟踪定位
    //设置跟踪设备参数NET_DVR_SET_TRACK_DEV_PARAMN
    public static final int NET_DVR_GET_TRACK_DEV_PARAM = 3261;//获取跟踪设备参数
    public static final int NET_DVR_SET_TRACK_DEV_PARAM = 3262 ;//设置跟踪设备参数
    
    //设置PTZ跟踪参数NET_DVR_SET_PTZ_TRACK_PARAM
    public static final int NET_DVR_GET_PTZ_TRACK_PARAM =3263 ;//获取PTZ跟踪参数
    public static final int NET_DVR_SET_PTZ_TRACK_PARAM =3264 ;//设置PTZ跟踪参数
    //云台花样扫描参数结构体NET_DVR_PTZ_PATTERN
    ...
    //获取区域入侵侦测配置
    public static final int NET_DVR_GET_FIELD_DETECTION  =3362 ;//获取区域入侵侦测配置
    public static final int NET_DVR_SET_FIELD_DETECTION   =3363 ;//设置区域入侵侦测配置
    //巡航预置点操作
    ...
    public static final int DEVICE_ALARM_ABILITY = 0x00a;// 设备报警能力 
    public static final int DEVICE_VIDEOPIC_ABILITY = 0x00e;// 设备图像参数能力 
    public static final int FISHEYE_ABILITY = 0x700;// 鱼眼IPC能力集 
    //NET_DVR_USER_V40用户参数配置（扩展）结构体。
    public static final int NET_DVR_GET_USERCFG_V40 = 6187;//获取用户参数
    public static final int NET_DVR_SET_USERCFG_V40 = 6188;//设置用户参数
    //启动远程配置。
    public static final int NET_DVR_RECORD_CHECK            = 6233;// 录像完整性检测 NET_DVR_RECORD_CHECK_COND NULL 
    public static final int NET_DVR_GET_RECORD_SEGMENT_CFG  = 6242;// 获取录像段总大小 NET_DVR_RECORD_SEGMENT_COND NULL 
    //录像完整性检测、获取录像段总大小
    public static final int NET_SDK_GET_NEXT_STATUS_SUCCESS = 1000;// 成功读取到数据，处理完本次数据后需要再次调用NET_DVR_GetNextRemoteConfig获取下一条数据 
    public static final int NET_SDK_GET_NETX_STATUS_NEED_WAIT = 1001;// 需等待设备发送数据，继续调用NET_DVR_GetNextRemoteConfig 
    public static final int NET_SDK_GET_NEXT_STATUS_FINISH = 1002;// 数据全部取完，可调用NET_DVR_StopRemoteConfig结束长连接 
    public static final int NET_SDK_GET_NEXT_STATUS_FAILED  = 1003;// 出现异常，可调用NET_DVR_StopRemoteConfig结束长连接 
    ...
    public static final int COMM_ALARM_V40 = 0x4007;// 移动侦测、视频丢失、遮挡、IO信号量等报警信息，报警数据为可变长 NET_DVR_ALARMINFO_V40 
...
//回调预览中的状态和消息
    ...
    /********************报警参数设置begin*********************/
    //穿越警戒面参数结构体。穿越方向，具体定义见下表： 
    public static final int VCA_BOTH_DIRECTION  = 0;//双向
    public static final int VCA_LEFT_GO_RIGHT   = 1;//由左至右
    public static final int VCA_RIGHT_GO_LEFT   = 2;//由右至左
    //越界侦测
    public static final int MAX_ALERTLINE_NUM = 8;//最大警戒线条数
    //区域入侵时用到
    public static final int MAX_INTRUSIONREGION_NUM = 8;//最大区域个数
    public static final int VCA_MAX_POLYGON_POINT_NUM = 10;// 检测区域最多支持10个点的多边形 
    /********************报警参数设置end*********************/
...
/* 设备类型 */
    ...
    public static final int IPDOME_MEGA200 = 41;     /*IP 200万高清快球*/
    public static final int IPDOME_MEGA130 = 42;     /*IP 130万高清快球*/
    ...
    public static final int IPCAM_FISHEYE = 1002; //鱼眼摄像机 
    public static final int IPCAM_BINOCULAR = 1004;// 双目摄像机 
...
/////////////////////////////////////////////////////////////////////////
    //设备通道参数结构体。鱼球联动时的联动跟踪设备（球机）参数 
    public static class NET_DVR_CHANNEL extends Structure {
        public byte[] byAddress = new  byte[MAX_DOMAIN_NAME];/*设备IP或域名*/	
        public short wDVRPort;/*设备端口号（默认：8000） */
        public byte[] byRes1 = new  byte[2];	/*保留，置为0 */
        public byte[] sUserName = new  byte[NAME_LEN];	/*设备用户名 */
        public byte[] sPassword= new  byte[PASSWD_LEN];/*设备密码 */
    ...
    //标定点子结构体。鱼球联动时用到
    public static class NET_DVR_CB_POINT extends Structure {
        public NET_VCA_POINT     struPoint;/*标定点，主摄像机（枪机），其值归一化到0-1  */
        public NET_DVR_PTZPOS    struPtzPos;/*球机输入的PTZ坐标  */
        public byte[] byRes = new  byte[8];	/*保留，置为0 */
    }
    //标定参数结构体。鱼球联动时用到
    public static class NET_DVR_TRACK_CALIBRATION_PARAM_V41 extends Structure {
        public byte byPointNum;/*有效标定点个数  */
        public byte[] byRes = new  byte[3];	/*保留，置为0 */
        public NET_DVR_CB_POINT[]  struCBPoint = new NET_DVR_CB_POINT[MAX_CALIB_PT];/*标定点组  */
        public NET_DVR_PTZPOS      struHorizonPtzPos;/*球机水平线PTZ坐标  */
        public byte[] byRes2 = new  byte[256];	/*保留，置为0 */
    }
    //跟踪设备参数结构。鱼球联动时用到
    public static class NET_DVR_TRACK_DEV_PARAM extends Structure {
        public int dwSize ;/*结构体大小  */
        public byte byEnable;/*是否启用此跟踪设备：0-不启用，1-启用   */
        public byte byTransMode ;/*通讯方式：0-网络SDK，1-485串口  */
        public byte[] byRes1 = new  byte[2];	/*保留，置为0 */
        public NET_DVR_CHANNEL  struTrackDevChan;/*联动跟踪设备参数  */
        public NET_DVR_TRACK_CALIBRATION_PARAM_V41    struCalParam;/*跟踪设备标定参数  */
        public byte[] byRes2 = new  byte[256];	/*保留，置为0 */
    }
    
    //手动跟踪定位参数结构体。
    public static class NET_DVR_PTZ_MANUALTRACE extends Structure {
        public int dwSize;/*结构体大小 */
        public int dwChannel;/*通道号 */
        public NET_VCA_POINT struPoint;/*定位坐标 */
        public byte byTrackType;/*跟踪类型：0- 普通跟踪，1- 高速道路跟踪，2- 城市道路跟踪 */
        public byte byLinkageType;/* 联动动作: 0- 手动跟踪，1- 联动不跟踪 */
        public byte[] byRes = new  byte[2];/*保留，置为0 */
        public NET_VCA_POINT struPointEnd;/*定位坐标终点 */
        public NET_DVR_TIME_V30  struTime;/*手动跟踪定位的当前时间，目前仅取证球机支持，和dwSerialNo一起用于平台录像和手动跟踪取证图片匹配使用 */
        public int dwSerialNo;/*抓拍序号，目前仅取证球机支持，和struTime一起用于平台录像和手动跟踪取证图片匹配使用 */
        public byte[] byRes1 = new  byte[36];/*保留，置为0*/
    }
    //PTZ跟踪参数结构。
    public static class NET_DVR_PTZ_TRACK_PARAM extends Structure {
        public int    dwSize;/*结构体大小 */
        public byte   byTrackMode;/*跟踪方式，具体定义如下
                                    PTZ_TRACK_MODE_ALWAYS   = 1   一直跟踪 
                                    PTZ_TRACK_MODE_BYTIME   = 2 跟踪指定时间 
                                    PTZ_TRACK_MODE_NEXT   = 3   跟踪下一目标  */
        public byte   byLinkageTarget;/*联动目标：0- 默认，1- 球机1，2- 球机2，…，依次类推  */
        public byte   byAutoTrackEnable;/*跟踪使能：0- 自动跟踪开启，1- 自动跟踪关闭（枪球设备才支持自动跟踪关闭功能，设备默认关闭）  */
        public byte   byRes1;/*保留，置为0*/
        public int    dwTrackTime;/*跟踪时间：
                                    当跟踪模式为PTZ_TRACK_MODE_ALWAYS，此字段无效
                                    当跟踪模式为PTZ_TRACK MODE_BYTIME时，范围：[1,60]，默认：10秒
                                    当跟踪模式为PTZ_TRACK MODE_NEXT时，范围：[1,10]，默认：2秒  */
        public byte[] byRes2 = new  byte[256];/*保留，置为0*/
    }
    //PTZ跟踪参数结构中的跟踪方式，具体定义如下
    public static class  PTZ_TRACK_MODE extends Structure {
        public static final int PTZ_TRACK_MODE_ALWAYS  = 1;
        public static final int PTZ_TRACK_MODE_BYTIME  = 2;
        public static final int PTZ_TRACK_MODE_NEXT    = 3;
    }
    //时间参数结构体
    public static class NET_DVR_TIME_V30 extends Structure {
        ...
        public String toString() {
            return "NET_DVR_TIME_V30.wYear: " + wYear + "\n" + "NET_DVR_TIME_V30.byMonth: \n" + byMonth + "\n" + "NET_DVR_TIME_V30.byDay: \n" + byDay + "\n" + "NET_DVR_TIME_V30.byHour: \n" + byHour + "\n" + "NET_DVR_TIME_V30.byMinute: \n" + byMinute + "\n" + "NET_DVR_TIME_V30.bySecond: \n" + bySecond + "\n" + "NET_DVR_TIME_V30.wMilliSec: \n" + wMilliSec;
        }
    }
    ...
    //错误信息列表；NET_DVR_GetDeviceConfig使用到
    public static class STATUSLIST  extends Structure {
        public int[] dwStatusList = new int[64];
    }
    //穿越警戒面参数结构体；越界侦测使用到
    public static class NET_VCA_TRAVERSE_PLANE extends Structure {
        public NET_VCA_LINE struPlaneBottom;/*警戒面底边  */
        public int dwCrossDirection;/*穿越方向，具体定义见下表： 
        ...
        public byte byPlaneHeight;/*警戒面高度（网络摄像机不支持该参数设置）  */
        public byte byDetectionTarget;/*检测目标：0- 所有目标，1- 人，2- 车   */
        public byte[]  byRes2 = new byte[37];/*保留，置为0   */
    }
    ...
    public static class NET_VCA_TRAVERSE_PLANE_DETECTION extends Structure {
        ...
        public NET_DVR_SCHEDTIMEWEEK[] struAlarmSched = new NET_DVR_SCHEDTIMEWEEK[MAX_DAYS]; /*布防时间，每周7天，每天最多设置8个时间段   */
        public NET_DVR_HANDLEEXCEPTION_V40    struHandleException;/*异常处理方式 */
        public int  dwMaxRelRecordChanNum;/*触发的录像通道数（只读）最大支持数量   */
        public int  dwRelRecordChanNum;/*报警触发的录像通道数实际支持数，即当前组包含的通道个数  */
        public int[] byRelRecordChan = new int[MAX_CHANNUM_V30];/*报警触发的录像通道，取数组前dwRelRecordChanNum个值，其值表示录像通道号。例如，dwRelRecordChanNum=5，则可以配置触发录像的通道：dwRelRecordChan[0]~dwRelRecordChan[4]。  */
        public NET_DVR_SCHEDTIME[]  struHolidayTime = new NET_DVR_SCHEDTIME[MAX_TIMESEGMENT_V30];/*假日布防时间，最多设置8个时间段   */
        public byte[]  byRes2 = new  byte[100];/*保留，置为0   */
        
    }
//    public static class NET_VCA_TRAVERSE_PLANE_DETECTION64 extends Structure {
//        public NET_VCA_TRAVERSE_PLANE_DETECTION[] strTreversePlane64 = new NET_VCA_TRAVERSE_PLANE_DETECTION[64];
//    }
    //通道号和组号信息结构体。通道参数配置时用到。越界侦测和区域入侵都用到
    public static class NET_DVR_CHANNEL_GROUP extends Structure {
        public int dwSize;/*结构体大小  */
        public int dwChannel;/*通道号  */
        public int dwGroup;/*组号，从0开始，即0表示第1组，1表示第2组，依次类推   */
        public byte byID;/*设备区域设置ID  */
        public byte[] byRes1 = new byte[3];/*保留，置为0   */
        public int  dwPositionNo;/*场景位置索引号，IPC为0，IPD从1开始   */
        public byte[] byRes = new byte[56];/*保留，置为0   */
    }
//    public static class NET_DVR_CHANNEL_GROUP64 extends Structure {
//        public NET_DVR_CHANNEL_GROUP[] strChannelGroup64 = new NET_DVR_CHANNEL_GROUP[64];
//    }

    //多边形结构体；区域入侵侦测使用到
    public static class NET_VCA_POLYGON extends Structure {
        public int   dwPointNum;/*有效点（大于等于3），若是3点在一条线上认为是无效区域，线交叉认为是无效区域   */
        public NET_VCA_POINT[]   struPos = new NET_VCA_POINT[VCA_MAX_POLYGON_POINT_NUM];/*多边形边界点，最大值为10  */
    }
    
    //入侵参数结构体；区域入侵侦测使用到
    public static class NET_VCA_INTRUSION extends Structure {
        public NET_VCA_POLYGON    struRegion;/*区域范围  */
        public short wDuration;/*行为事件触发时间阈值，判断有效报警的时间。不同的设备取值范围不同，智能服务器为1~120秒（默认：5秒），ATM设备为1~1200秒，Smart IPC为1～100秒   */
        public byte bySensitivity;/*灵敏度参数，取值范围：[1,100]   */
        public byte byRate;/*占比：区域内所有未报警目标尺寸目标占区域面积的比重，归一化为1~100   */
        public byte byDetectionTarget;/*检测目标：0- 所有目标，1- 人，2- 车   */
        public byte[] byRes = new byte[3];/*保留，置为0   */
    }
    
    //区域入侵侦测配置结构体；区域入侵侦测使用到
    public static class NET_VCA_FIELDDETECION extends Structure {
        public int  dwSize;/*结构体大小  */
        public byte  byEnable;/*使能区域入侵侦测功能：0- 否，1- 是   */
        public byte   byEnableDualVca;/*启用支持智能后检索：0- 不启用，1- 启用   */
        public byte[] byRes1 = new byte[2];/*保留，置为0   */
        public NET_VCA_INTRUSION[]  struIntrusion= new NET_VCA_INTRUSION[MAX_INTRUSIONREGION_NUM];/*区域参数   */
        public NET_DVR_SCHEDTIMEWEEK[] struAlarmSched = new NET_DVR_SCHEDTIMEWEEK[MAX_DAYS]; /*布防时间，每周7天，每天最多设置8个时间段   */
        public NET_DVR_HANDLEEXCEPTION_V40    struHandleException;/*异常处理方式  */
        public int  dwMaxRelRecordChanNum;/*触发的录像通道数（只读）最大支持数量   */
        public int  dwRelRecordChanNum;/*报警触发的录像通道数实际支持数，即当前组包含的通道个数   */
        public int[] byRelRecordChan = new int[MAX_CHANNUM_V30];/*报警触发的录像通道，取数组前dwRelRecordChanNum个值，其值表示录像通道号。
                                    例如，dwRelRecordChanNum=5，则可以配置触发录像的通道：byRelRecordChan[0]~byRelRecordChan[4]。   */
        public NET_DVR_SCHEDTIME[]  struHolidayTime = new NET_DVR_SCHEDTIME[MAX_TIMESEGMENT_V30];/*假日布防时间，最多设置8个时间段   */
        public byte[] byRes2 = new byte[100];/*保留，置为0   */
    ...
    //校时结构参数
    public static class NET_DVR_TIME extends Structure implements Comparable<NET_DVR_TIME>{//校时结构参数
        public int dwYear;		//年
        ...
        @Override
        public int compareTo(NET_DVR_TIME o) {
            if      (dwYear     > o.dwYear)     return 1;
            else if (dwYear     < o.dwYear)     return -1;
            else if (dwMonth    > o.dwMonth)    return 1;
            else if (dwMonth    < o.dwMonth)    return -1;
            else if (dwDay      > o.dwDay)      return 1;
            else if (dwDay      < o.dwDay)      return -1;
            else if (dwHour     > o.dwHour)     return 1;
            else if (dwHour     < o.dwHour)     return -1;
            else if (dwMinute   > o.dwMinute)   return 1;
            else if (dwMinute   < o.dwMinute)   return -1;
            else if (dwSecond   > o.dwSecond)   return 1;
            else if (dwSecond   < o.dwSecond)   return -1;
            else                                return 0;
        }
        
        public int compareTo(NET_DVR_TIME_EX o) {
            if      (dwYear     > o.wYear)     return 1;
            else if (dwYear     < o.wYear)     return -1;
            else if (dwMonth    > o.byMonth)    return 1;
            else if (dwMonth    < o.byMonth)    return -1;
            else if (dwDay      > o.byDay)      return 1;
            else if (dwDay      < o.byDay)      return -1;
            else if (dwHour     > o.byHour)     return 1;
            else if (dwHour     < o.byHour)     return -1;
            else if (dwMinute   > o.byMinute)   return 1;
            else if (dwMinute   < o.byMinute)   return -1;
            else if (dwSecond   > o.bySecond)   return 1;
            else if (dwSecond   < o.bySecond)   return -1;
            else                                return 0;
        }
    }
    
    //时间参数结构体。录像完整性检测条件参数结构体NET_DVR_RECORD_CHECK_COND中用到
    ...
    public static class NET_DVR_STREAM_INFO extends Structure {
        ...
        public  int     dwSize;                         /*结构体大小*/
        public  byte[]  byID = new byte[STREAM_ID_LEN]; /*流ID，为字母、数字和"_"的组合。全部为0时，无效 */
        public  int     dwChannel;                      /*关联的设备通道。等于0xffffffff时，如果是设置流的来源信息(NET_DVR_SET_STREAM_SRC_INFO)，表示不关联；
    ...
    public static class NET_DVR_RECORD_CHECK_COND extends Structure {
        ...
        public  int                     dwSize;             /*结构体大小*/
        public  NET_DVR_STREAM_INFO     struStreamInfo;     /*流信息（仅流ID有效） */
        public  byte                    byCheckType;        /*检测方式：0- 录像是否完整，1- 录像是否完整&缺失录像的起止时间 */
        public  byte[]                  byRes1 = new byte[3];/*保留，置为0 */
        public  NET_DVR_TIME_EX         struBeginTime;      /*检测录像的开始时间 */
        public  NET_DVR_TIME_EX         struEndTime;        /*检测录像的结束时间 */
    ...
    public static class NET_DVR_RECORD_CHECK_RET extends Structure {
        ...
        public  byte[]               byRes1 = new byte[3];/*保留，置为0 */
        public  NET_DVR_TIME_EX    struBeginTime;/*缺失录像的开始时间，byRecordNotComplete参数值和NET_DVR_RECORD_CHECK_COND查询条件中的byCheckType都为1时有效 */
        public  NET_DVR_TIME_EX    struEndTime;/*缺失录像的结束时间，byRecordNotComplete参数值和NET_DVR_RECORD_CHECK_COND查询条件中的byCheckType都为1时有效*/
        public  byte[]               byRes = new byte[128];/*保留，置为0 */
    }
    ...
    public static class NET_DVR_PLAYCOND extends Structure {
        ...
        public  byte[]        byStreamID = new byte[STREAM_ID_LEN]; /*流ID，使用流ID方式时dwChannel设为0xffffffff  */
        public  byte[]        byRes = new byte[30]; /*保留，置为0 */
    }
...
//网络数据结构(子结构)(9000扩展)
 ...
 public static class NET_DVR_PICCFG extends Structure { //通道图象结构(SDK_V13及之前版本)
    ...
    public static class NET_DVR_RECORD_V30 extends Structure {    //通道录像参数配置(9000扩展)
        ...
        public byte  byStreamType;      /*码流类型：0- 主码流，1- 子码流，3- 第三码流  */
        public byte  byPassbackRecord;  /*0- 不回传录像，1- 回传录像  */
        public short wLockDuration;     /* 录像锁定时长，单位：小时，0表示不锁定，0xffff表示永久锁定。录像段的时长大于锁定的持续时长的录像，将不会锁定 */
        ...
        public byte[] byReserve = new byte[2];  /*保留，置为0 */
    }
    ...
    public static class NET_DVR_SCHED_CAPTURECFG extends Structure {
        ...
        public NET_DVR_RECORDSCHEDWEEK[]    struCaptureSched = new NET_DVR_RECORDSCHEDWEEK[MAX_DAYS];/*时间段抓图布防计划，数组0~6分别对应星期一~星期日  */
        public NET_DVR_CAPTURE_DAY          struCaptureHoliday;/*假日抓图计划  */
        public NET_DVR_CAPTURE_SCHED[]      struHolidaySched = new NET_DVR_CAPTURE_SCHED[MAX_TIMESEGMENT_V30];/*假日抓图时间段布防计划 */
        public int                          dwRecorderDuration;/*抓图保存最长时间，单位：天，0xffffffff表示该值无效  */
        public byte[]                       byReserve = new byte[40];/*保留 */
    }
    
    
    //全天抓图计划结构体
    public static class NET_DVR_CAPTURE_DAY extends Structure {
        public byte    byAllDayCapture;/*是否全天抓图 */
        public byte    byCaptureType;/*抓图类型：0-定时抓图，1-移动侦测抓图，2-报警抓图，3-移动侦测或报警抓图，4-移动侦测和报警抓图，6-智能报警抓图，7-智能侦测抓图，8-全部事件 */
        public byte[]  byRes = new byte[2];/* 保留 */
    }
    //时间段抓图布防计划结构体
    public static class NET_DVR_CAPTURE_SCHED extends Structure {
...
public static class NET_DVR_USER_INFO_V40 extends Structure {//单用户参数(子结构)(9000扩展)
	public byte[] sUserName = new byte[NAME_LEN];		/* 用户名 */
	public byte[] sPassword = new byte[PASSWD_LEN];		/* 密码 */
        public byte[] byLocalRight = new byte[MAX_RIGHT];	/* 本地权限 */
                                                                /*数组0: 本地控制云台*/
	...
	public byte[] byRemoteRight = new byte[MAX_RIGHT];  /* 远程权限 */
                                                            ...
                                                            /*数组12: 远程管理模拟和IP camera */
                                                            /*数组13: 远程关机/重启 */
	public int[] byNetPreviewRight = new int[MAX_CHANNUM_V40];		/* 远程可以预览的通道 0-有权限，1-无权限*/
	public int[] byLocalPlaybackRight = new int[MAX_CHANNUM_V40];	    /* 本地可以回放的通道 0-有权限，1-无权限*/
	public int[] byNetPlaybackRight = new int[MAX_CHANNUM_V40];	    /* 远程可以回放的通道 0-有权限，1-无权限*/
	public int[] byLocalRecordRight = new int[MAX_CHANNUM_V40];		/* 本地可以录像的通道 0-有权限，1-无权限*/
	public int[] byNetRecordRight = new int[MAX_CHANNUM_V40];		    /* 远程可以录像的通道 0-有权限，1-无权限*/
	public int[] byLocalPTZRight = new int[MAX_CHANNUM_V40];		    /* 本地可以PTZ的通道 0-有权限，1-无权限*/
	public int[] byNetPTZRight = new int[MAX_CHANNUM_V40];			/* 远程可以PTZ的通道 0-有权限，1-无权限*/
	public int[] byLocalBackupRight = new int[MAX_CHANNUM_V40];		/* 本地备份权限通道 0-有权限，1-无权限*/
	public NET_DVR_IPADDR struUserIP;		/* 用户IP地址(为0时表示允许任何地址) */
	public byte[] byMACAddr = new byte[MACADDR_LEN];	/* 物理地址 */
	public byte byPriority;				/* 优先级，0xff-无，0--低，1--中，2--高 */
                                                        /*
                                                        ...
                                                        高……管理员
                                                        */
        public byte byAlarmOnRight;	/*报警输入口布防权限：0-无权限，1-有权限  */
        public byte byAlarmOffRight;	/*报警输入口撤防权限：0-无权限，1-有权限  */
        public byte byBypassRight;	/*报警输入口旁路权限：0-无权限，1-有权限  */
	public byte[] byRes = new byte[118]; /* 保留，置为0  */
}
...
public static class NET_DVR_USER_V40 extends Structure {
	public int dwSize;/*结构体大小 */
        public int dwMaxUserNum;/*设备支持的最大用户个数（只读） */
	public NET_DVR_USER_INFO_V40[] struUser = new NET_DVR_USER_INFO_V40[MAX_USERNUM_V30];/*用户信息，struUser[0]~struUser[dwMaxUserNum-1]有效  */
}
...
*/
    ...
    */
	...
	public int dwSize;                              /* 结构体大小 */
        ...
        public byte[] byRes1 = new byte[2];             /* 保留，置为0  */
        public byte[] byName = new byte[NAME_LEN];      /* 预置点名称 */
        public byte[] byRes = new byte[64];             /* 保留，置为0  */
    }
    public static class NET_DVR_PRESET_NAME320 extends Structure {
        public NET_DVR_PRESET_NAME[] strPreSetName320 = new NET_DVR_PRESET_NAME[320];
    }
    /****************************DS9000新增结构(end)******************************/
    //完全恢复出厂值控制参数
    public static class NET_DVR_COMPLETE_RESTORE_INFO extends Structure {
        public int    dwSize;                      /* 结构体大小 */
        public int    dwChannel;                   /* 设备通道号，目前该参数无效  */
        public byte[] byRes = new byte[64];        /* 保留，置为0  */
    }
...
public static class NET_DVR_CRUISE_RET extends Structure {
    public NET_DVR_CRUISE_POINT[] struCruisePoint = new NET_DVR_CRUISE_POINT[CRUISE_MAX_PRESET_NUMS];			//最大支持32个巡航点
}
//云台花样扫描参数结构体。
public static class NET_DVR_PTZ_PATTERN extends Structure {
    public int      dwSize;         /* 结构体大小 */
    public int      dwChannel;      /* 通道号 */
    public int      dwPatternCmd;   /* 云台轨迹操作命令码，见下表： 宏定义 宏定义值 含义 
                                        STA_MEM_CRUISE 34 开始记录轨迹 
                                        STO_MEM_CRUISE 35 停止记录轨迹 
                                        RUN_CRUISE 36 开始运行轨迹 
                                        STOP_CRUISE 44 停止运行轨迹 
                                        DELETE_CRUISE 45 删除单条轨迹 
                                        DELETE_ALL_CRUISE 46 删除所有轨迹  */
    public int      dwPatternID;    /* 云台轨迹ID（删除所有轨迹时无效）  */
    public byte[]   byRes = new byte[64];   /* 保留，置为0  */
}
//云台守望参数结构体
public static class NET_DVR_PTZ_PARKACTION_CFG extends Structure {

    public int      dwSize;                 /*结构体大小*/
    public byte     byEnable;               /*是否启用：0-不启用，1-启用*/
    public byte[]   byRes1 = new byte[3];   /*保留，置为0  */
    public int      dwParkTime;             /*守望等待时间，单位：s */
    public short    wActionType;            /*守望动作类型：0-自动扫描，1-帧扫描，2-随机扫描，3-巡航扫描，4-花样扫描，5-预置点，6-全景扫描，7-垂直扫描*/
    public short    wID;                    /*ID号（巡航扫描、预置点、花样扫描的ID） */
    public byte[]   byRes = new byte[128];  /*保留，置为0  */
}
/************************************多路解码器(begin)***************************************/
...
public static class NET_DVR_DEVICEINFO_V30 extends Structure
{
    ...
    public  short wDevType;/*设备型号.推荐使用wDevType作为设备类型。 如果byDVRType是0，则接口中解析wDevType作为设备型号；如果byDVRType非0，则接口中byDVRType和wDevType值相等，都是byDVRType。*/
    public  byte bySupport3;/**/
    public  byte byMultiStreamProto;/**/
    public  byte byStartDChan;/*起始数字通道号，0表示无数字通道，比如DVR或IPC */
    public  byte byStartDTalkChan;/*起始数字对讲通道号，区别于模拟对讲通道号，0表示无数字对讲通道 */
    public  byte byHighDChanNum;/*数字通道个数，高8位 */
    public  byte bySupport4;/**/
    public  byte byLanguageType;/*支持语种能力，按位表示，位与结果：0- 不支持，1- 支持*/
    public  byte byVoiceInChanNum;/*音频输入通道数*/
    public  byte byStartVoiceInChanNo;/*音频输入起始通道号，0表示无效 */
    public  byte[] byRes3 = new byte[2];/*保留，置为0 */
...
//NET_DVR_Login_V40()参数结构。设备参数结构体。
public static class NET_DVR_DEVICEINFO_V40 extends Structure
{
    ...
    public byte    byRes1;                /*保留，置为0 */
    public int     dwSurplusLockTime;     /*剩余时间，单位：秒，用户锁定时此参数有效。在锁定期间，用户尝试登陆，不管用户名密码输入对错，设备锁定剩余时间重新恢复到30分钟 */
    ...
    public byte[]  byRes2 = new byte[255];/*保留，置为0 */
}

//NET_DVR_Login_V40()用户登录参数结构体。
public static class NET_DVR_USER_LOGIN_INFO extends Structure
{
    public byte[]  sDeviceAddress = new byte[NET_DVR_DEV_ADDRESS_MAX_LEN];/*设备地址，IP 或者普通域名 */
    public byte    byRes1;                                                /*保留，设为 0 */
    public short   wPort;                                                 /*设备端口号，例如：8000 */
    public byte[]  sUserName = new byte[NET_DVR_LOGIN_USERNAME_MAX_LEN];  /*登录用户名，例如：admin*/
    public byte[]  sPassword = new byte[NET_DVR_LOGIN_PASSWD_MAX_LEN];    /*登录密码，例如：12345 */
    public FLoginResultCallBack    cbLoginResult;                         /*登录状态回调函数，bUseAsynLogin 为1时有效 */
    public Pointer pUser;                                                 /*用户数据 */
    public byte    bUseAsynLogin;                                         /*是否异步登录：0- 否，1- 是 */
    public byte[]  byRes2 = new byte[128];                                /*保留，置为0 */
}
//sdk网络环境枚举变量，用于远程升级
 enum _SDK_NET_ENV
{
...
//抓图模式
 //抓图模式
     public static class  CAPTURE_MODE
    {
            public static final int BMP_MODE = 0;		//BMP模式
            public static final int JPEG_MODE = 1;		//JPEG模式
    }
 ...
 enum REALSOUND_MODE
{
    ...
    public static class VCA_RULE_EVENT_TYPE_EX{
        public final static short ENUM_VCA_EVENT_TRAVERSE_PLANE     = 1;  /*穿越警戒面*/
        public final static short ENUM_VCA_EVENT_ENTER_AREA         = 2;  /*目标进入区域，支持区域规则*/
        public final static short ENUM_VCA_EVENT_EXIT_AREA          = 3;  /*目标离开区域，支持区域规则*/
        public final static short ENUM_VCA_EVENT_INTRUSION          = 4;  /*周界入侵，支持区域规则*/
        public final static short ENUM_VCA_EVENT_LOITER             = 5;  /*徘徊，支持区域规则*/
        public final static short ENUM_VCA_EVENT_LEFT_TAKE          = 6;  /*丢包捡包，支持区域规则*/
        public final static short ENUM_VCA_EVENT_PARKING            = 7;  /*停车，支持区域规则*/
        public final static short ENUM_VCA_EVENT_RUN                = 8;  /*快速移动(奔跑)，支持区域规则*/
        public final static short ENUM_VCA_EVENT_HIGH_DENSITY       = 9;  /*区域内人员密度，支持区域规则*/
        public final static short ENUM_VCA_EVENT_VIOLENT_MOTION     = 10;  /*剧烈运动检测*/
        public final static short ENUM_VCA_EVENT_REACH_HIGHT        = 11;  /*攀高检测*/
        public final static short ENUM_VCA_EVENT_GET_UP             = 12;  /*起身检测*/
        public final static short ENUM_VCA_EVENT_LEFT               = 13;  /*物品遗留*/
        public final static short ENUM_VCA_EVENT_TAKE               = 14;  /*物品拿取*/
        public final static short ENUM_VCA_EVENT_LEAVE_POSITION     = 15;  /*离岗*/
        public final static short ENUM_VCA_EVENT_TRAIL              = 16;  /*尾随*/
        public final static short ENUM_VCA_EVENT_KEY_PERSON_GET_UP  = 17;  /*重点人员起身检测*/
        public final static short ENUM_VCA_EVENT_STANDUP            = 18;  /*起立检测*/
        public final static short ENUM_VCA_EVENT_FALL_DOWN          = 20;  /*倒地检测*/
        public final static short ENUM_VCA_EVENT_AUDIO_ABNORMAL     = 21;  /*声强突变检测*/
        public final static short ENUM_VCA_EVENT_ADV_REACH_HEIGHT   = 22;  /*折线攀高*/
        public final static short ENUM_VCA_EVENT_TOILET_TARRY       = 23;  /*如厕超时*/
        public final static short ENUM_VCA_EVENT_YARD_TARRY         = 24;  /*放风场滞留*/
        public final static short ENUM_VCA_EVENT_ADV_TRAVERSE_PLANE = 25;  /*折线警戒面*/
        public final static short ENUM_VCA_EVENT_HUMAN_ENTER        = 29;  /*人靠近ATM（仅在ATM_PANEL模式下支持）*/
        public final static short ENUM_VCA_EVENT_OVER_TIME          = 30;  /*操作超时（仅在ATM_PANEL模式下支持）*/
        public final static short ENUM_VCA_EVENT_STICK_UP           = 31;  /*贴纸条,支持区域规则*/
        public final static short ENUM_VCA_EVENT_INSTALL_SCANNER    = 32;  /*安装读卡器,支持区域规则*/
        public final static short ENUM_VCA_EVENT_PEOPLENUM_CHANGE   = 35;  /*人数变化事件*/
        public final static short ENUM_VCA_EVENT_SPACING_CHANGE     = 36;  /*间距变化事件*/
        public final static short ENUM_VCA_EVENT_COMBINED_RULE      = 37;  /*组合规则事件*/
        public final static short ENUM_VCA_EVENT_SIT_QUIETLY        = 38;  /*一动不动（静坐）事件*/
...
//软解码预览参数
    public static class NET_DVR_CLIENTINFO extends Structure {
        ...
        @Override
        public String toString() {
            return "NET_DVR_CLIENTINFO.lChannel: " + lChannel.toString() + "\r\n" + "NET_DVR_CLIENTINFO.lLinkMode: " + lLinkMode + "\r\n" + "NET_DVR_CLIENTINFO.hPlayWnd: " + hPlayWnd.toString() + "\r\n" + "NET_DVR_CLIENTINFO.sMultiCastIP: " + sMultiCastIP ;
        }
    }
...
public static class NET_DVR_FINDDATA_CARD extends Structure
{
 ...
 public static class NET_DVR_FILECOND extends Structure //录象文件查找条件结构
    ...
    /*注册回放裸码流数据回调函数，用到该结构*/
    public static class NET_DVR_PACKET_INFO_EX extends Structure //数据包信息结构体
    {
        public short          wWidth;       /* 宽度 */  
        public short          wHeight;      /* 高度*/  
        public int            dwTimeStamp;  /* 时间戳低位*/  
        public int            dwTimeStampHigh; /* 时间戳高位*/  
        public int            dwYear;       /* OSD时间：年 */  
        public int            dwMonth;      /* OSD时间：月 */  
        public int            dwDay;        /*OSD时间：日  */  
        public int            dwHour;        /* OSD时间：时 */  
        public int            dwMinute;        /*OSD时间：分  */  
        public int            dwSecond;        /* OSD时间：秒 */  
        public int            dwMillisecond;   /* OSD时间：毫秒*/  
        public int            dwFrameNum;      /*帧号 */  
        public int            dwFrameRate;     /* 帧率，当帧率小于0时，0x80000002表示1/2帧率，同理可推0x80000010为1/16帧率 */  
        public int            dwFlag;        /* flag E帧标记 */  
        public int            dwFilePos;        /* 帧在文件中的位置 */  
        public int            dwPacketType;        /* 包类型：0- 文件头，1- 视频I帧，2- 视频B帧， 3- 视频P帧， 10- 音频数据包， 11- 私有数据包 */  
        public int            dwPacketSize;        /*包数据大小 */  
        public String         pPacketBuffer;        /* 包数据缓冲区指针*/  
        public byte[]         byRes1 = new byte[4]; /* 保留，置为0 */  
        public int            dwPacketMode;        /* 打包方式：0- 保留，1- FU_A打包方式*/  
        public byte[]         byRes2 = new byte[16]; /* 保留，置为0  */  
        public int[]          dwReserved = new int[6]; /* 私有数据信息，保留 */  
    }
...
public static class RECV_ALARM extends Structure{
    ...
    //登录状态回调函数(NET_DVR_Login_V40)
    public static interface FLoginResultCallBack extends StdCallCallback {
        public void invoke(NativeLong lUserID, int dwResult, Pointer lpDeviceInfo, Pointer pUser);//lpDeviceInfo NET_DVR_DEVICEINFO_V30  
    }
 /***API函数声明,详细说明见API手册***/
...
//××××××××××××××××××××××开始：在本次编程中已经使用的或者肯定要使用的××××××××××××××××××××××××
    //初始化SDK，调用其他SDK函数的前提。
    boolean  NET_DVR_Init();
    boolean  NET_DVR_Cleanup();//释放SDK资源，在程序结束之前调用。在调用NET_DVR_Cleanup的时候，不能同时调用其他任何SDK接口。NET_DVR_Init和NET_DVR_Cleanup需要配对使用，即程序里面调用多少次NET_DVR_Init，退出时就需要调用多少次NET_DVR_Cleanup。

    int  NET_DVR_GetLastError();
    //××××××××××××××××××××××结束：在本次编程中已经使用的或者肯定要使用的××××××××××××××××××××××××
    ...
    //用户注册设备（支持异步登录）。pLoginInfo [in] 登录参数，包括设备地址、登录用户、密码等 lpDeviceInfo [out] 设备信息(同步登录即pLoginInfo中bUseAsynLogin为0时有效) 
   ...
   //2007-04-16增加查询结果带卡号的文件查找
    ...
    //参数配置 begin
     ...
     //获取设备能力集
     boolean NET_DVR_GetDeviceAbility( NativeLong lUserID, int dwAbilityType, String pInBuf, int dwInLength, byte[] pOutBuf, int dwOutLength);
